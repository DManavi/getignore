...
pack-nodejs:
	...
	zip $(ARTIFACT_DIR)/$(NODEJS_FILE_NAME).zip -r dist package.json package-lock.json

pack-macos-x64:
	mkdir -p $(ARTIFACT_DIR)
	cp $(OUTPUT_DIR)/getignore-cmd-macos-x64 $(ARTIFACT_DIR)/$(MACOS_X64_FILE_NAME)
	zip $(ARTIFACT_DIR)/$(PACKAGE_PREFIX)-macos_x64.zip $(ARTIFACT_DIR)/$(MACOS_X64_FILE_NAME)
	rm $(ARTIFACT_DIR)/$(MACOS_X64_FILE_NAME)

pack-linux-x64:
	mkdir -p $(ARTIFACT_DIR)
	cp $(OUTPUT_DIR)/getignore-cmd-linux-x64 $(ARTIFACT_DIR)/$(LINUX_X64_FILE_NAME)
	zip $(ARTIFACT_DIR)/$(PACKAGE_PREFIX)-linux_x64.zip $(ARTIFACT_DIR)/$(LINUX_X64_FILE_NAME)
	rm $(ARTIFACT_DIR)/$(LINUX_X64_FILE_NAME)

pack-linux-arm64:
	mkdir -p $(ARTIFACT_DIR)
	cp $(OUTPUT_DIR)/getignore-cmd-linux-arm64 $(ARTIFACT_DIR)/$(LINUX_ARM64_FILE_NAME)
	zip $(ARTIFACT_DIR)/$(PACKAGE_PREFIX)-linux_arm64.zip $(ARTIFACT_DIR)/$(LINUX_ARM64_FILE_NAME)
	rm $(ARTIFACT_DIR)/$(LINUX_ARM64_FILE_NAME)

pack-windows-x64:
	mkdir -p $(ARTIFACT_DIR)
	cp $(OUTPUT_DIR)/getignore-cmd-win-x64.exe $(ARTIFACT_DIR)/$(WINDOWS_X64_FILE_NAME).exe
	zip $(ARTIFACT_DIR)/$(PACKAGE_PREFIX)-windows_x64.zip $(ARTIFACT_DIR)/$(WINDOWS_X64_FILE_NAME).exe
...
publish-github: pack-nodejs pack-macos-x64 pack-linux-x64 pack-linux-arm64 pack-windows-x64 tag-source
	...
	gh release upload $(VERSION_NAME) '$(ARTIFACT_DIR)/$(NODEJS_FILE_NAME).zip'
